...
\begin{figure}[H]
    ...
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{1em}
        ...
        Excalidraw. Attori: ``Utente anonimo'', ``Utente registrato'' e
        ``Amministratore'';
...
\begin{enumerate}
    ...
    \item L'app invia \endpoint{GET}{/api/get/<id>} e mostra il testo.
\end{enumerate}
...
\begin{enumerate}
    ...
    crea il record e restituisce l'\emph{access token} JWT.
    \item L'app salva il token nello storage sicuro e mostra la home da
    autenticato.
\end{enumerate}
...
% =====================================================================
\paragraph{UC7 --- Gestione del catalogo (Amministratore)}\hfill\\
\textbf{Scope:} moderazione contenuti.\\
\textbf{Level:} user-goal.\\
\textbf{Intention in context:} l'amministratore vuole visionare e moderare
l'intero catalogo, inclusi i brani anonimi che un utente normale non può
gestire.\\
\textbf{Primary actor:} Amministratore.

\textbf{Precondition:} l'amministratore è autenticato con un account
avente \code{is\_admin = true}.\\
\textbf{Minimum guarantees:} un utente non-admin non può accedere alle
funzioni di amministrazione.\\
\textbf{Success guarantees:} l'amministratore visualizza il catalogo
filtrato e può eliminare qualsiasi brano.

\subparagraph{Main success scenario:}
\begin{enumerate}
    \item L'amministratore apre la schermata ``Admin''.
    \item L'app invia \endpoint{GET}{/admin/songs} con il JWT in header.
    \item Il backend verifica \code{is\_admin}; se falso risponde 403.
    \item Il backend restituisce l'elenco completo dei brani, inclusi
    quelli con \code{user\_id = NULL} (anonimi).
    \item L'amministratore può filtrare per testo (\code{q}), per brani
    anonimi (\code{anonymous=true}) o per utente (\code{user\_id}).
    \item Per eliminare un brano, l'app invia
    \endpoint{DELETE}{/api/songs/<id>}; il backend accetta la richiesta
    poiché rileva il flag \code{is\_admin}.
\end{enumerate}

\subparagraph{Extensions:}
\begin{itemize}
    \item \textbf{3a:} JWT mancante o scaduto $\Rightarrow$ 401.
    \item \textbf{3b:} utente autenticato ma non admin $\Rightarrow$ 403.
\end{itemize}
