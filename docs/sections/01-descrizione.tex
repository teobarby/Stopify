...
\begin{itemize}
    ...
    sulla cache locale.
    \item È sviluppata anche un'applicazione mobile cross-platform (iOS,
    Android, Web) basata su React Native ed Expo che funge da client
    principale per la consultazione e la pubblicazione.
    \item L'autenticazione è basata su \emph{JSON Web Token} (JWT) con
    \emph{access} e \emph{refresh token}.
    ...
    backup, monitoraggio) sono trattate come \emph{future work} e dichiarate
    esplicitamente nel capitolo dedicato alla sicurezza.
\end{itemize}
