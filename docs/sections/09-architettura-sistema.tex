...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}lX@{}}
        ...
        \code{src/api.ts} & Client REST con auto-refresh del JWT. \\
...
\begin{itemize}
    \item \code{users} \emph{(id, username, email, password\_hash, created\_at)} ---
    utenti registrati. Username ed email univoci (case-insensitive).
    \item \code{artists} \emph{(id, name, created\_at)} --- artisti, nome
    univoco.
    \item \code{albums} \emph{(id, title, year, artist\_id, created\_at)} ---
